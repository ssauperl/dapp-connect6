...
contract ConnectSix {
  ...
  function new_game(uint _time_per_move, uint opponent_stake) public {
    games.length++;
  ...
  function join_game(uint game_num) public {
  ...
  function player_1(uint game_num) public view returns (address) {
    return games[game_num].players[1];
  }
  
  function player_2(uint game_num) public view returns (address) {
  ...
  function move_history(uint game_num) public view returns (uint8[] memory) {
  ...
  function make_move(uint game_num, uint8 x1, uint8 y1, uint8 x2, uint8 y2) public {
    Game memory g = games[game_num];
  ...
  function make_move_and_claim_victory(uint game_num, uint8 x1, uint8 y1, uint8 x2, uint8 y2, uint8 wx, uint8 wy, uint8 dir) public {
  ...
  function claim_victory(uint game_num, uint8 x, uint8 y, uint8 dir) public {
    Game memory g = games[game_num];
    if (x > board_size 
        || y > board_size
        || g.winner != 0
